...
io.on("connection", (socket) => {
  ...
  socket.on("join", (nickname) => {
    ...
    if (existing && existing.id !== null) {
      // Someone with same nickname is already connected
      socket.emit("joinError", "A player with this name is already connected.");
      return;
    }
  ...
  socket.on("manualDisconnect", () => {
    ...
    io.emit("players", players);
    savePlayers();
    socket.disconnect(); // ניתוק בפועל מהשרת
  });
  ...
  socket.on("disconnect", () => {
    ...
    if (disconnected) {
      disconnected.id = null; // השאר בזיכרון לשיחזור עתידי
    }
